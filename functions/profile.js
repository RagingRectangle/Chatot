...
module.exports = {
    ...
    async function sendProfileChangeMessage(profileList) {
      try {
        ...
        for (var p = 0; p < profileList.length; p++) {
          ...
          let areaInfo = areaArray.length > 0 ? `\n- **${locale.profileAreas}:** ${areaArray.join(', ')}` : ``;
        ...
        var profileDropdown = new StringSelectMenuBuilder()
          ...
          .setPlaceholder(locale.profileChangeDescription);
  ...
  changeProfile: async function changeProfile(client, interaction, config, util, humanInfo) {
    ...
    if (newProfileName == oldProfileName) {
      await interaction.reply({
        content: `${locale.profileChanged}: **${newProfileName.replace('✅','')}**.`,
        ephemeral: true
      });
      ...
      superagent
        ...
        .end((error, response) => {
          if (error) {
            ...
            interaction.reply({
              content: `${locale.profileChanged}: **${newProfileName}**`,
              ephemeral: true
            }).catch(console.error);
            updateProfileMessage();
          }
